var jsPsychSurveyTextEdited = (function (jspsych) {
  ...
  class SurveyTextPlugin {
      ...
      trial(display_element, trial) {
        ...
        // add submit button
        html +='<button id="show_tuple" class="jspsych-btn jspsych-survey-text">Ready to see graph</button>'

        // function showGraph() { 
        //     var x = document.getElementById("jspsych-survey-text-form"); 
        ...
        html += "<img src='" + 'images_2/' + trial.graphID + "' class='graph_images'></img>"
        ...
        html += '<div class="row">'
          ...
          for (var i = 0; i < trial.questions.length; i++) {
              ...
              "</p><p style='font-size: 15px !important; margin-bottom: 2px; \
              font-style: italic'>Write your answer in the units of the y-axis. \
              You may use decimals to convey your answer. Please try to be as accurate as possible!</p>";
              ...
              if (question.rows == 1) {
                  html +=
                    //   '<input type="text" id="input-' +
                      ...
                      step="0.0001" id="input-0" name="#jspsych-survey-text-response-' +
                          question_index +
                          '" data-name="' +
                          question.name +
                          '" size="' +
                  ...
                  html +=
                      '<textarea id="input-0" name="#jspsych-survey-text-response-' +
                          question_index +
                          '" data-name="' +
                          question.name +
                          '" cols="' +
          ...
          // delay display of UI by 1 second
          setTimeout(() => {
            ...
            var startTime_show_tuple;

            display_element.querySelector("#show_tuple").addEventListener("click", function() {
                startTime_show_tuple = performance.now();
                $("#jspsych-survey-text-form").show()
                $("#show_tuple").hide()
            });

            var startTime_typing;

            // tracking when participant starts typing
            function timer() {
                startTime_typing = performance.now();
                console.log('startTime_typing', startTime_typing) 
                // stop listening
                display_element.querySelector("#input-0").removeEventListener("input", timer)
            }

             // connect tracker to eventListener
            display_element.querySelector("#input-0").addEventListener("input", timer)

          display_element.querySelector("#jspsych-survey-text-form").addEventListener("submit", (e) => {
              e.preventDefault();

              // measure response time
              var submitTime = performance.now();
              var response_time = Math.round(submitTime - startTime);
          ...
          if (trial.practice == 'practice') {
            var trialdata = {
                ...
                startTime: startTime,
                startTime_show_tuple: startTime_show_tuple,
                startTime_typing: startTime_typing,
                endTime: submitTime, 
                rt: response_time,

...
function checkPreviousParticipant(workerId, callback) {
  ...
  var postData = {
    dbname: 'davinci_exp2_receiver', //'davinci_cogsci_receiver',
    query: p,
    projection: {
      '_id': 1
    }
...
function initializeWithTrials(socket) {
  var gameid = UUID();
  var colname = 'davinci_exp2_receiver'; // davinci_cogsci_receiver
  sendPostRequest('http://localhost:7033/db/getbatchstims', {
    json: {
      dbname: 'stimuli',
      colname: colname,
      //numTrials: 1,
